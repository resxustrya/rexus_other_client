    ...
    function form($name,$userid,$date_from,$date_to,$sched)
    {
        ...
        if(isset($sched) and count($sched) > 0) {
            ...
            if(count($logs) <= 0) {
                ...
                /*
                $this->Image(__DIR__.'/image/logo.png', 15, 50,80,80);
                $this->Image(__DIR__.'/image/logo.png', 118, 50,80,80);
                */
                ...
                $this->SetXY(10,47);
                $this->Cell(89,5,'  DAY    ARRIVAL | DEPARTURE   ARRIVAL | DEPARTURE LATE|UT|OT',1);
                    ...
                    for($r1 = $startday; $r1 <= $endday; $r1++)
                    {
                        ...
                        if($datein == $log_date)
                        {
                            ...
                            if(!$log['holiday'] == '001' OR !$log['holiday'] == '003' OR !$log['holiday'] == '002') {
                                ...
                                $ot = overtime($s_am_in,$s_pm_in,$am_in,$pm_in,$s_am_out,$s_pm_out,$am_out,$pm_out,$datein);
                                if($ot != '' or $ot != null)
                                {
...
function overtime($s_am_in,$s_pm_in,$am_in,$pm_in,$s_am_out,$s_pm_out,$am_out,$pm_out,$datein)
{

    $total = null;

    if (isset($pm_out) AND strtotime($pm_out) < '24:00:00' AND strtotime($pm_out) > strtotime($s_pm_out)) {
